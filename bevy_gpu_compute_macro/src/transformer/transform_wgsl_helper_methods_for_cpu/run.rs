...
struct HelperFunctionConverterForCpu {
    custom_types: Vec<CustomType>,
}

impl VisitMut for HelperFunctionConverterForCpu {
...
impl HelperFunctionConverterForCpu {
...
pub fn transform_wgsl_helper_methods_for_cpu(state: &mut ModuleTransformState) {
    assert!(
    ...
    let custom_types = if let Some(ct) = &state.custom_types {
        ...
        abort!(
            state.rust_module_for_cpu.ident.span(),
            "Allowed types must be set before transforming helper functions"
        );
    };
    let mut converter = HelperFunctionConverterForCpu::new(custom_types);
    converter.visit_item_mod_mut(&mut state.rust_module_for_cpu);
}
