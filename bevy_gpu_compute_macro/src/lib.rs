...
use transformer::{
    custom_types::get_all_custom_types::get_custom_types, module_parser::lib::{parse_library_module, parse_shader_module},
...
#[proc_macro_attribute]
#[proc_macro_error]
pub fn wgsl_library_module(_attr: TokenStream, item: TokenStream) -> TokenStream {
    set_dummy(item.clone().into());
    let content = item.to_string();
    let module = parse_macro_input!(item as syn::ItemMod);
    DocCommentRemover {}.visit_item_mod(&module);
    ...
    transform_wgsl_helper_methods(&mut state);
    transform_wgsl_helper_methods_for_cpu(&mut state);
    parse_library_module(&mut state);
    let output = produce_expanded_output(&mut state);
    output.into()
}
