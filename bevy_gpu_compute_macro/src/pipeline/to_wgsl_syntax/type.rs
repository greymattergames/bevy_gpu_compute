...
use crate::pipeline::custom_types::custom_type::CustomType;

pub struct TypeToWgslTransformer<'a> {
    pub custom_types: &'a Vec<CustomType>,
}
impl VisitMut for TypeToWgslTransformer<'_> {
    fn visit_type_path_mut(&mut self, t: &mut syn::TypePath) {
        syn::visit_mut::visit_type_path_mut(self, t);
        path_type_to_wgsl(t, self.custom_types);
    }
} //

pub fn path_type_to_wgsl(type_path: &mut syn::TypePath, custom_types: &[CustomType]) {
...
fn convert_path_segment(segment: PathSegment, custom_types: &[CustomType]) -> PathSegment {
    ...
    if custom_t.is_some() {
        ...
        match ident.to_string().as_str() {
            ...
            _ => {
                let message = format!("Unsupported type in type_to_wgsl: {}", ident);
