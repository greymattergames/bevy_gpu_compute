...
override _LIB_WORKGROUP_SIZE_X: u32 = 64;
override _LIB_WORKGROUP_SIZE_Y: u32 = 1;
override _LIB_WORKGROUP_SIZE_Z: u32 = 1;
override POSITION_INPUT_ARRAY_LENGTH: u32 = 1;
override RADIUS_INPUT_ARRAY_LENGTH: u32 = 1;
override COLLISIONRESULT_OUTPUT_ARRAY_LENGTH: u32 = 1;
struct Config { time : f32, resolution : vec2 < f32 > , }
struct Position { pub v : vec2 < f32 > , }
// !make types capital
alias position_input_array  = array < Position, POSITION_INPUT_ARRAY_LENGTH > ;
alias Radius  = f32;
alias radius_input_array  = array < Radius, RADIUS_INPUT_ARRAY_LENGTH > ;
struct CollisionResult { entity1 : u32, entity2 : u32, }alias collisionresult_output_array  = array < CollisionResult,
COLLISIONRESULT_OUTPUT_ARRAY_LENGTH > ;

//! what is this doing?
collisionresult_counter@group(0) @binding(0) var<uniform, read> config: Config;

@group(0) @binding(1) var<storage, read> position_input_array: position_input_array;

@group(0) @binding(2) var<storage, read> radius_input_array: radius_input_array;

@group(0) @binding(3) var<storage, read_write> collisionresult_output_array: 
...
fn main(@builtin(global_invocation_id) iter_pos: vec3<u32>)
{
 ...
 if current_radius <= 0.0
    || other_radius <= 0.0 { return;
 } let current_pos = position_input_array
    [current_entity];
 let other_pos = position_input_array [other_entity];
 let
    dist_squared = calculate_distance_squared(current_pos.v, other_pos.v);
 let
    radius_sum = current_radius + other_radius;
 if dist_squared < radius_sum *
    radius_sum
    {
        {
            let collisionresult_output_array_index =
            atomicAdd(& collisionresult_counter, 1u);
 if
