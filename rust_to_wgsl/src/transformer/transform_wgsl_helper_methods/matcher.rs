        ...
        match (&method.category, &method.method) {
            (WgslHelperCategory::VecInput, WgslHelperMethodName::VecLen) => {
                assert!(
                    ...
                    "Expected {} to be an input array type, since WgslVecInput::vec_len is called. Put #[wgsl_input_array] above your type declaration to fix this. A given type cannot be used for multiple purposes, for example a type T cannot be both an input array and an output array.",
            ...
            (WgslHelperCategory::VecInput, WgslHelperMethodName::VecVal) => {
                ...
                assert!(
                    method.t_def.kind == CustomTypeKind::InputArray,
                    "Expected {} to be an input array type, since WgslVecInput::vec_val is called. Put #[wgsl_input_array] above your type declaration to fix this. A given type cannot be used for multiple purposes, for example a type T cannot be both an input array and an output array.",
                    method.t_def.name.name
                );
            ...
            (WgslHelperCategory::Output, WgslHelperMethodName::Len) => {
                assert!(
                    ...
                    "Expected {} to be an output array or vec type, since WgslOutput::len is called. Put #[wgsl_output_array] or #[wgsl_output_vec] above your type declaration to fix this. A given type cannot be used for multiple purposes, for example a type T cannot be both an input array and an output array.",
                    method.t_def.name.name
                );
            ...
            (WgslHelperCategory::Output, WgslHelperMethodName::Set) => {
                assert!(
                    method.t_def.kind == CustomTypeKind::OutputArray,
                    "Expected {} to be an output array type, since WgslOutput::set is called. Put #[wgsl_output_array] above your type declaration to fix this. A given type cannot be used for multiple purposes, for example a type T cannot be both an input array and an output array.",
                    method.t_def.name.name
                );
                assert!(
