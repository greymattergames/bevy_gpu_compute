...
pub fn wgsl(stream: TokenStream1) -> TokenStream1 {
    let input = parse_macro_input!(stream as );

    match input {
        WgslInput::Type(ty) => {
            // Handle type-based generation
            wgsl2(quote! {
                derive_struct_definition(input)
            })
        }
        WgslInput::Literal(tokens) => {
            // Original wgsl! macro implementation for literals
            wgsl2(tokens)
        }
    }
    .into()
        ...
        Ok(module) => {
            match Validator::new(ValidationFlags::all(), Capabilities::all()).validate(&module) {
                ...
                Err(e) => {
                    ...
                    let e_str = format!("validation error: {}", e.to_string());
...
/// Derive macro to generate WGSL struct definitions
// #[proc_macro_derive(WgslStruct)]
// #[proc_macro_error]
pub fn derive_struct_definition(input: TokenStream1) -> TokenStream1 {
    ...
    let fields = match input.data {
        ...
        _ => abort!(
            Span::call_site(),
            "WgslStruct can only be derived for structs"
        ),
    };

    // Generate WGSL field definitions
    let field_defs = match fields {
        ...
        _ => abort!(Span::call_site(), "WgslStruct only supports named fields"),
    };
